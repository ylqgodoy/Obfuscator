...
class ylqgOBF:
    ...
    def encrypt1(self):
        ...
        init = ";".join(init)
        self.code = f'''
# Encriptado usando https://github.com/ylqgodoy/Obfuscator
...
'''.strip().encode()
    ...
    def encrypt2(self):
        ...
        self.code = f'''# Encriptado usando https://github.com/ylqgodoy/Obfuscator
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog= sys.argv[0], description= "Encriptar um arquivo python")
    parser.add_argument("FILE", help= "Caminho para o arquivo que contém o código py")
    parser.add_argument("-o", type= str, help= 'Local de saida do arquivo [Padrçao: "Encriptado_<FILE>.py"]', dest= "path")
    args = parser.parse_args()

    if not os.path.isfile(sourcefile := args.FILE):
        printerr(f'Arquivo inexistente: "{args.FILE}"')
        os._exit(1)
    elif not sourcefile.endswith((".py", ".pyw")):
        printerr('O arquivo não possui uma extensão de py válida!')
        os._exit(1)
    
    if args.path is None:
        args.path = "Encriptado_" + os.path.basename(sourcefile)
    
    with open(sourcefile) as sourcefile:
        code = sourcefile.read()
    
    ylqgOBF(code, args.path)